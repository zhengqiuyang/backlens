"""Train here, deploy anywhere: BackLens -> ONNX -> onnxruntime.

Trains a classifier in BackLens, exports it to a standard ONNX file, runs it
...
from backlens import Tensor, MLP, Tanh, Adam
...
def main():
    ...
    print("training in BackLens (numpy, CPU)...")
    ...
    report = verify_onnx(model, X[:1], path, n_inputs=5)
    print(report.report())
    assert report
    ...
    print(f"onnxruntime accuracy: {ort_acc:.2%}")
    assert ort_acc == acc

    # ---------------------------------------------------------------
    # full circle: load the exported file back into BackLens
    # ---------------------------------------------------------------
    from backlens.onnx_loader import load_onnx
    reloaded = load_onnx(path)
    back = reloaded(Tensor(X)).data
    print(f"roundtrip (BackLens -> ONNX -> BackLens) max diff: "
