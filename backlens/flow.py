...
# ---------------------------------------------------------------------------
# GradientFlowTimeline: how parameter shares migrate over training
# ---------------------------------------------------------------------------

class GradientFlowTimeline:
    """Track gradient shares across a training run.

    Call :meth:`snapshot` every N training steps (BEFORE ``opt.step()``, so
    the measured backward pass matches the gradients the optimizer will use;
    re-running backward on the same tape recomputes identical gradients).
    :meth:`save_html` renders a stacked-area chart of the shares over time --
    the story of "who eats the gradient" as training progresses.
    ...
    Example:
        ...
        for step in range(200):
            ...
            opt.step()
        tl.save_html("flow_timeline.html")
    """

    def __init__(self):
        self.snapshots: List[dict] = []

    def snapshot(self, loss: Tensor, step: Optional[int] = None) -> GradientFlow:
        """Measure one backward pass and file it into the timeline."""
        flow = gradient_flow(loss)
        self.snapshots.append({
            "step": len(self.snapshots) if step is None else int(step),
            "shares": {p["name"]: p["share"] for p in flow.params},
        })
        return flow

    def _series(self) -> Tuple[List[str], np.ndarray]:
        """(ordered param names, matrix of shares: snapshots x params)."""
        names: List[str] = []
        for snap in self.snapshots:
            for n in snap["shares"]:
                if n not in names:
                    names.append(n)
        order = sorted(names, key=lambda n: -self.snapshots[-1]["shares"].get(n, 0.0))
        mat = np.array([[s["shares"].get(n, 0.0) for n in order]
                        for s in self.snapshots])
        return order, mat

    def report(self) -> str:
        names, mat = self._series()
        lines = [f"gradient flow timeline: {len(self.snapshots)} snapshots"]
        for i, n in enumerate(names):
            first, last = mat[0, i], mat[-1, i]
            lines.append(f"  {n:<26} {first:5.1f}% -> {last:5.1f}%")
        return "\n".join(lines)

    def save_html(self, path: str, title: str = "gradient flow over training") -> None:
        names, mat = self._series()
        steps = [s["step"] for s in self.snapshots]
        data = {"title": title, "names": names, "steps": steps,
                "matrix": [[_fin(v) for v in row] for row in mat]}
        payload = json.dumps(data).replace("</", "<\\/")
        html = _TIMELINE_TEMPLATE.replace("__TL_JSON__", payload)
        with open(path, "w", encoding="utf-8") as f:
            f.write(html)


_TIMELINE_TEMPLATE = r"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>BackLens gradient flow timeline</title>
<style>
  :root { --bg:#0d1117; --panel:#161b22; --border:#30363d; --fg:#c9d1d9;
  ...
  .card { background:var(--panel); border:1px solid var(--border);
          border-radius:8px; padding:12px 16px; margin:14px 20px; }
...
<body>
<header>
  <h1><span class="lens">BackLens</span> gradient flow over training</h1>
  <div id="meta"></div>
</header>
<div class="card"><div id="chart"></div>
  <table id="legend"></table>
</div>
<footer>generated by <code>GradientFlowTimeline.save_html</code> &mdash; single file, zero dependencies</footer>
<script>
const DATA = __TL_JSON__;
const PALETTE = ['#58a6ff','#3fb950','#d29922','#f85149','#bc8cff',
                 '#39c5cf','#ff7b72','#7ee2a8','#ffa657','#79c0ff'];
document.getElementById('meta').textContent =
  `${DATA.title}  ·  ${DATA.names.length} parameters  ·  ${DATA.steps.length} snapshots`;

const W = 1000, H = 320, PADL = 46, PADB = 26, PADT = 10;
const maxStep = Math.max(...DATA.steps, 1);
const xOf = i => PADL + (i / Math.max(1, DATA.steps.length - 1)) * (W - PADL - 14);
const yOf = pct => PADT + (1 - pct / 100) * (H - PADT - PADB);

const svgNS = 'http://www.w3.org/2000/svg';
const svg = document.createElementNS(svgNS, 'svg');
svg.setAttribute('viewBox', `0 0 ${W} ${H + PADB}`);
// y grid at 0/50/100
[0, 50, 100].forEach(p => {
  const ln = document.createElementNS(svgNS, 'line');
  ln.setAttribute('x1', PADL); ln.setAttribute('x2', W - 14);
  ln.setAttribute('y1', yOf(p)); ln.setAttribute('y2', yOf(p));
  ln.setAttribute('stroke', '#21262d');
  svg.appendChild(ln);
  const t = document.createElementNS(svgNS, 'text');
  t.setAttribute('x', 8); t.setAttribute('y', yOf(p) + 4);
  t.setAttribute('fill', '#8b949e'); t.setAttribute('font-size', '11');
  t.textContent = p + '%';
  svg.appendChild(t);
});
// stacked areas (cumulative shares)
const cum = new Array(DATA.steps.length).fill(0);
DATA.names.forEach((name, pi) => {
  const top = [], bot = [];
  DATA.steps.forEach((_, i) => {
    bot.push([xOf(i), yOf(cum[i])]);
    cum[i] += DATA.matrix[i][pi] || 0;
    top.push([xOf(i), yOf(cum[i])]);
  });
  ...
  path.setAttribute('fill', PALETTE[pi % PALETTE.length]);
...
let row = null;
DATA.names.forEach((name, pi) => {
  if (pi % 3 === 0) { row = document.createElement('tr'); legend.appendChild(row); }
  const first = DATA.matrix[0][pi], last = DATA.matrix[DATA.matrix.length - 1][pi];
  const td = document.createElement('td');
  td.innerHTML = `<span class="sw" style="background:${PALETTE[pi % PALETTE.length]}"></span>` +
    `${name} &nbsp; ${first.toFixed(1)}% -> ${last.toFixed(1)}%`;
  row.appendChild(td);
});
</script>
</body>
